...
birthday_year = generator.GenerateFuncAttribute(
                                  ...
                                  parameters = [35, "triangular", False, "DD/MM/YYYY"]
                                  )
...
placeholder_corruptor = corruptor.CorruptPlaceholder()


attr_name_list = ['given-name', 'second-name', 'surname','telephone-number',
                  'birthday_year']

attr_data_list = [given_name_attr, second_name_attr, surname_attr, phone_number_attribute, birthday_year]

attr_mod_prob_dictionary = {'given-name':0.05,
                            'second-name':0.05,
                            'surname':0.05,
...
attr_mod_data_dictionary = {#'surname':[(0.5, surname_misspell_corruptor),
                            ...
                            #'given-name':[(0.5, edit_corruptor2),
                                          #(0.5, ocr_corruptor)],
                            'given-name':[(1.0, placeholder_corruptor)],
                            'second-name':[(1.0, placeholder_corruptor)],
                            'surname':[(1.0, placeholder_corruptor)],
...
#
rec_dict = test_data_corruptor.corrupt_names(rec_dict=rec_dict, type='switch', prob=0.7, first_col_index=0, second_col_index=1, third_col_index=2)

test_data_generator.rec_dict = rec_dict
